...
buycommoncratebutton = InlineKeyboardButton('Poke Crate (500 Pokedollars)',callback_data='buycrate:poke')
buyuncommoncratebutton = InlineKeyboardButton('Great Crate (1000 Pokedollars)',callback_data='buycrate:great')
buyrarecratebutton = InlineKeyboardButton('Ultra Crate (5000 Pokedollars)',callback_data='buycrate:ultra')
buylegendarycratebutton = InlineKeyboardButton('Master Crate (20000 Pokedollars)',callback_data='buycrate:master')
...
def crateshopcallback(update, context):
    ...
    if query.data == 'buycrate:poke':
        ...
        if not inventory.userinventory[uid]['pokedollars'] < 1000:
            ...
            inventory.add_pokedollars(uid,-1000)
            inventory.add_balls(uid,'pokeballs',pokeballsnum)
            inventory.add_balls(uid,'greatballs',greatballsnum)
        else:
            query.edit_message_text('Sorry, but you cannot afford this item.')
    elif query.data == 'buycrate:ultra':
        if not inventory.userinventory[uid]['pokedollars'] < 5000:
        ...
        if not inventory.userinventory[uid]['pokedollars'] < 50000:
            ...
            inventory.add_pokedollars(uid,-50000)
            inventory.add_balls(uid,'pokeballs',pokeballsnum)
            inventory.add_balls(uid,'greatballs',greatballsnum)
            inventory.add_balls(uid,'ultraballs',ultraballsnum)
